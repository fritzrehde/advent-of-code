...
class Board:
    ...
    def wins_after_mark(self, marked_num: int) -> bool:
        """
...
def solve(puzzle_input: str) -> str:
    ...
    for number in numbers:
        for board in boards:
            if board.wins_after_mark(number):
                unmarked_sum = sum(
                    map(lambda cell: cell.num, board.get_unmarked_cells())
                )
                result = unmarked_sum * number
                return str(result)

    raise Exception("unreachable: assume one of the boards wins")
